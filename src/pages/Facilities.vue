  ...
  <div
    ...
    <div
      ...
      <div
        ...
        :key="key" class="facility-item group animate__animated animate__fadeInUp" :class="{ 'animate__delay-4s': getFirstEnter === true }">
        <img
    ...
    <!-- 小標題 -->
    <div ref="block_observatory_target" class="w-10/12 mt-28 mb-0">
      <h2
    ...
    <!-- 篩選列 -->
    <div
      class="table-filter w-10/12 mt-14 mb-8 inline-flex animate__animated animate__faster"
      :class="{ 'animate__fadeIn': block_observatory_target_isVisible }">
      <ul v-if="observatoryCategories" class="flex flex-wrap">
        <li v-for="(val, key) in observatoryCategories" :key="key">
          <div class="flex items-center mr-10 group">
            <input
              :id="String(val.observatory_category_id)" v-model="selectCat" type="radio" name="radio"
              class="hidden" :value="val.observatory_category_id" />
            <label
              :for="String(val.observatory_category_id)" :class="[
    ...
    <div
      ...
      :class="{ 'animate__fadeIn': block_observatory_target_isVisible }">
      <v-md-preview
        v-for="(val, key) in observatoryCategories" v-show="selectCat === val.observatory_category_id"
        :key="key" class="animate__animated animate__fadeIn animate__faster"
        :text="val.observatory_post_content"></v-md-preview>
    </div>
  </div>
  <Footer />
</template>
