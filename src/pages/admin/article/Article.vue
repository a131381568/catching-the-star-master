<template>
	<div class="flex items-stretch">
		...
		<div
			...
			<div class="w-9/12 mobile:w-11/12 flex justify-between mb-20 mobile:mb-9 mobile:block mobile:mx-auto">
				<h1 class="text-white relative -left-2 -top-2 mobile:text-5xl w-3/4 mobile:w-full">文章列表</h1>
				<router-link to="/board/article/add"
			...
			<div class="w-9/12 mobile:w-11/12 table-container mobile:m-auto">
				<table id="responsive-table" v-if="artistsPagiEdges.length > 0 && artistsPagiEdges"
					...
					<tbody>
						<tr v-for="(val, key) in artistsPagiEdges" :key="key">
							<td data-title="標題">{{
									val.title
							}}</td>
	...
	c0.5-0.5,1-1.1,1.6-1.6c1,1,2,2,3,3c-4.5,4.6-9,9.3-13.6,13.9c-0.5,0.5-1,1.1-1.6,1.6c-1-1-2-2-3-3C8.5,13.1,13,8.5,17.6,3.9z
	 M3.4,19.2c0.7,0.7,1.5,1.5,2.2,2.2c-0.7,0.3-1.4,0.6-2.2,1c-0.3-0.3-0.7-0.6-1-1C2.8,20.7,3.1,19.9,3.4,19.2z" />
							...
							<td data-title="刪除">
								<svg @click.prevent="setConfirmModal(Number(val.postid))" xmlns="http://www.w3.org/2000/svg"
									viewBox="0 0 25 25"
				...
				<Pagination @selectPagi="selectPagi" :hasNextPage="Boolean(artistsPagiPageInfo.hasNextPage)"
					:hasPreviousPage="Boolean(artistsPagiPageInfo.hasPreviousPage)" :totalPagi="artistsPagiPageInfo.totalPagi"
...
import { artistsPagi, artistsCategories, deletePost } from '../../../api/science'
import { StargazingArr, PageInfoPush, ArtistsArr, ArtistConnectionPush, ArtistsCategories } from '../../../types/graphql/types'
...
const artistsPagiEdges = ref<ArtistsArr>([])
const artistsPagiPageInfo = ref<PageInfoPush>({
	hasNextPage: false,
...
const pidRef = ref("")
const postCatList = ref<ArtistsCategories>([])

// ======================= 生命週期 ======================= 

onMounted(async () => {
	// 取得列表資訊
	await getArtistsPagi(1)
	// 取得分類對照表
...
async function getArtistsPagi(pagi: Number) {
	const res = await artistsPagi(Number(pagi), 10, routeName)
	artistsPagiEdges.value = []
	setTimeout(() => {
		artistsPagiEdges.value = res.data.artistsPagi.edges
		artistsPagiPageInfo.value = res.data.artistsPagi.pageInfo
	}, 300);
}

// 跳出燈箱詢問是否確定刪除?
function setConfirmModal(pid: number) {
	pidRef.value = pid
	store.openPopMsg("確定刪除此文章?", true)
}
...
async function popBtnCheck() {
	...
	if (popBtnCheckVal.value) {
		// 刪除函式
		await deletePost(pidRef.value, routeName)
		// await deleteStargazer(pidRef.value, routeName)
		// 刪除後重抓列表, 並跳至第一頁
		await getArtistsPagi(1)
	}
}

function editPlace(pid: string) {
	router.push("/board/article/edit/" + pid)
}
...
async function getArtistsCategories() {
	...
	postCatList.value = res.data.artistsCategories
}
</script>